...
class Agent:
	...
	def run(self, MDP, weight_matrix, greedy):
		...
		while reward != MDP.final_reward:
			...
			weight_matrix, eligibility = Lambda_update(previous_position, action, action_matrix, position, reward, self.gamma, self.alpha, self.lambd, eligibility, self.q_table, weight_matrix)
...
def Lambda_update(state, action, action_matrix, new_state, reward, gamma, alpha, lambd, eligibility, phi_s, weight_matrix):
	...
	left_new_state = np.matmul(weight_matrix[0].T, np.cos(np.pi * np.dot(phi_s, new_state + [1, 0, 0])))
	neutral_new_state = np.matmul(weight_matrix[1].T, np.cos(np.pi * np.dot(phi_s, new_state + [0, 1, 0])))
	right_new_state = np.matmul(weight_matrix[2].T, np.cos(np.pi * np.dot(phi_s, new_state + [0, 0, 1])))

	if left_new_state == np.max([left_new_state, neutral_new_state, right_new_state]):
		error = reward + gamma*left_new_state - state_phi_s
	elif neutral_new_state == np.max([left_new_state, neutral_new_state, right_new_state]):
		error = reward + gamma*neutral_new_state - state_phi_s
	else:
...
def main():
	...
	print("number of trials =", trials, "epsilon =", epsilon, "alpha =", alpha, "lambda =", lambd)
