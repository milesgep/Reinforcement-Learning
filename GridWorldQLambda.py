...
class Agent:
	...
	def run(self, MDP, greedy):
		...
		while self.final_state != MDP.position:
			...
			self.q_table[previous_position-1][action], eligibility = Lambda_update(previous_position-1, action, position-1, reward, self.gamma, self.alpha, self.lambd, eligibility, self.q_table)
...
def evaluate(number_of_episodes, gamma, alpha, epsilon, lambd, fourier_basis, greedy):
...
def Lambda_update(state, action, new_state, reward, gamma, alpha, lambd, eligibility, phi_s):
	eligibility[state][action] = lambd*eligibility[state][action] + 1

	action_value = phi_s[state][action]
	next_action_value = np.max(phi_s[new_state])
...
def main():
	...
	#alphas = np.random.uniform(0.000001, 0.001, 3)
	#epsilons = np.random.uniform(0.1, 1.0, 3)
	#lambdas = np.random.uniform(0.00001, 0.01, 3)

	# best values so far
	epsilon = 0.3777023073910126 
	alpha = 0.0007814825529398007 
	lambd = 0.00012877408842087797
	...
	print("number of episodes =", number_of_episodes, "number of trials =", trials, "epsilon =", epsilon, "alpha =", alpha, "lambda =", lambd)
	for trial in range(trials):
		q_table, trial_data = evaluate(number_of_episodes, gamma, alpha, epsilon, lambd, fourier_basis, greedy)
