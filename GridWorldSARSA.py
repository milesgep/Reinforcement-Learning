...
class Agent:
	...
	def run(self, MDP, greedy):
		...
		while self.final_state != MDP.position:
			...
			self.time_step += 1
			if greedy:
				next_action = self.make_epsilon_greedy_move(next_position)
			else:
			...
			self.q_table[position-1][action] = q_update(self.q_table, position-1, action, next_position-1, next_action, reward, self.gamma, self.alpha)
...
def q_update(q_table, state, action, new_state, new_action, reward, gamma, alpha):
...
def main():
	...
	#alphas = np.random.uniform(0.01, 1.0, 5)
	#epsilons = np.random.uniform(1.0, 10.0, 5)

	# Greedy graphed values
	...
	#plt.errorbar(range(len(all_trial_data[0])), np.mean(all_trial_data, axis=0), yerr=np.std(all_trial_data, axis=0), color='yellow', ecolor='yellow', fmt='o')
	plt.plot(range(number_of_episodes), np.mean(all_trial_data, axis=0), color='yellow', label="Epsilon Greedy")

	# Soft max values
	epsilon = 1.798218923072791
	alpha = 0.9144446744503891
	greedy = False
	...
	#plt.errorbar(range(len(all_trial_data[0])), np.mean(all_trial_data, axis=0), yerr=np.std(all_trial_data, axis=0), color='green', ecolor='green', fmt='o')
