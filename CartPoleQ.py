...
class Agent:
	...
	def make_greedy_move(self, state, weight_matrix):
		left = [1, 0]
		right = [0, 1]
		left_action = (state + left)
		right_action = (state + right)

		left_state = np.matmul(weight_matrix.T, np.cos(np.pi * np.dot(self.q_table, left_action)))
		right_state = np.matmul(weight_matrix.T, np.cos(np.pi * np.dot(self.q_table, right_action)))
	...
	def run(self, MDP, weight_matrix, greedy):
		...
		while time_step != MDP.max_time:
			...
			position, reward, next_time_step = MDP.change_state(action)
			weight_matrix = TD_update(previous_position, action_matrix, position, reward, self.gamma, self.alpha, self.q_table, weight_matrix)
			self.response(reward, time_step)
...
def evaluate(number_of_episodes, fourier_basis, gamma, alpha, epsilon, greedy):
	q_table = make_CP_phi(fourier_basis)
	expected_returns = []
...
def TD_update(state, action, new_state, reward, gamma, alpha, phi_s, weight_matrix):
	state_phi_s = np.matmul(weight_matrix.T, np.cos(np.pi * np.dot(phi_s, state + action)))
	left_new_state = np.matmul(weight_matrix.T, np.cos(np.pi * np.dot(phi_s, new_state + [1, 0])))
	right_new_state = np.matmul(weight_matrix.T, np.cos(np.pi * np.dot(phi_s, new_state + [0, 1])))

	if left_new_state > right_new_state:
		error = reward + gamma*left_new_state - state_phi_s
	else:
		error = reward + gamma*right_new_state - state_phi_s

	weight = weight_matrix.T + (error*alpha*np.cos(np.pi * np.dot(phi_s, state + action))).T
	return weight.T

def make_CP_phi(fourier_basis):
	phi_s = []
	for basis_1 in range(fourier_basis):
		for basis_2 in range(fourier_basis):
			for basis_3 in range(fourier_basis):
				for basis_4 in range(fourier_basis):
					for basis_5 in range(fourier_basis):
						for basis_6 in range(fourier_basis):
...
def main():
	...
	#alphas = np.random.uniform(0.00001, 0.01, 5)
	#epsilons = np.random.uniform(0.3, 0.6, 5)

	# graphed epsilon greedy values
	epsilon = 0.0011954304361084945 
	alpha = 6.62786039621009e-06
	fourier_basis = 3
	greedy = True
	...
	for trial in range(trials):
		trial_data = evaluate(number_of_episodes, fourier_basis, gamma, alpha, epsilon, greedy)
	...
	epsilon = 0.5970231898013623 
	alpha = 0.001917376906724099
	greedy = False
	#fourier_basis = 3
